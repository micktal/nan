...
import { exportToCSV, exportToPDF, exportToJSON, calculateAnalytics } from '@/utils/export-utils';
...
const generateMockSessions = (): TrainingSession[] => {
  ...
  const companies = ['Transport ABC', 'Maintenance Pro', 'CleanCorp', 'AdminServices', 'TechSolutions'];
  ...
  return Array.from({ length: 50 }, (_, i) => {
    ...
    return {
      ...
      progress: {
        ...
        completedSteps: isCompleted ? [0, 1, 2, 3, 4, 5] : Array.from({ length: Math.floor(Math.random() * 4) + 1 }, (_, j) => j),
...
export default function AdminDashboard() {
  ...
  const profileStats = filteredSessions.reduce((acc, session) => {
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-white">{filteredAnalytics.totalSessions}</p>
                  <p className="text-emerald-400 text-sm">+{Math.round(Math.random() * 20)}% ce mois</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-white">{filteredAnalytics.averageScore.toFixed(1)}%</p>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-white">{Math.round(filteredAnalytics.averageDuration)}</p>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-white">
                    {Math.round((filteredAnalytics.completedSessions / filteredAnalytics.totalSessions) * 100)}%
                  </p>
                  <p className="text-purple-400 text-sm">{filteredAnalytics.completedSessions} terminées</p>
                </div>
              ...
              <div className="space-y-4">
                {completionTrend.map((day, index) => (
                  <div key={index} className="flex items-center justify-between">
                    <span className="text-slate-300 text-sm w-12">{day.date}</span>
                    <div className="flex-1 mx-4">
                      <Progress value={(day.completions / Math.max(...completionTrend.map(d => d.completions))) * 100} className="h-2" />
                    </div>
                    <span className="text-white text-sm w-8 text-right">{day.completions}</span>
                  </div>
                ))}
              </div>
            </CardContent>
            ...
            <CardContent>
              <div className="space-y-4">
                {Object.entries(profileStats).map(([profile, count]) => {
                  const percentage = (count / filteredAnalytics.totalSessions) * 100;
                  const profileLabels = {
                    driver: 'Chauffeurs',
                  ...
                  return (
                    <div key={profile} className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span className="text-slate-300">{profileLabels[profile as keyof typeof profileLabels] || profile}</span>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-700">
                    <th className="text-left text-slate-400 p-3">Utilisateur</th>
                    <th className="text-left text-slate-400 p-3">Profil</th>
                    <th className="text-left text-slate-400 p-3">Entreprise</th>
                    <th className="text-left text-slate-400 p-3">Progression</th>
                ...
                <tbody>
                  {filteredSessions.slice(0, 10).map((session) => {
                    ...
                    return (
                      <tr key={session.sessionId} className="border-b border-slate-700/50">
                        <td className="text-white p-3">
                          {user ? `${user.firstName} ${user.lastName}` : 'N/A'}
                        </td>
                        <td className="text-slate-300 p-3">{user?.profileType || 'N/A'}</td>
                        <td className="text-slate-300 p-3">{user?.company || 'N/A'}</td>
                        <td className="p-3">
                        ...
                        <td className="p-3">
                          {progress?.qcmScore ? (
                            <span className={`px-2 py-1 rounded text-sm ${
                              progress.qcmScore >= 80 ? 'bg-emerald-500/20 text-emerald-400' :
                              progress.qcmScore >= 60 ? 'bg-orange-500/20 text-orange-400' :
                        ...
                        <td className="text-slate-300 p-3">
                          {progress ? Math.round(progress.sessionDuration / 60) : 0} min
                        </td>
                        <td className="p-3">
                          {progress?.certificateGenerated ? (
                            <span className="text-emerald-400 text-sm">✅ Terminé</span>
                          ) : (
