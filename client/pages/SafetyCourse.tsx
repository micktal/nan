              ...
              {selectedZone ? (
                    ...
                    {(() => {
                      ...
                      return (
                        <>
                          ...
                          <p className="text-slate-300 mb-4">{zone?.description}</p>
                ...
                <Card className="bg-slate-800/50 border-slate-600/50">
                  <CardContent className="p-6 text-center">
                    ...
                    <p className="text-slate-300">
                      Cliquez sur une zone pour afficher les informations de sécurité
                    </p>
                  </CardContent>
                </Card>
              )}
                  ...
                  <div className="space-y-2">
                    {safetyZones.map((zone) => (
                      <div key={zone.id} className="flex items-center">
                        <div className={`w-4 h-4 rounded-full mr-3 ${completedZones.includes(zone.id) ? 'bg-emerald-500' : 'bg-slate-600'}`} />
                        <span className={completedZones.includes(zone.id) ? 'text-emerald-400' : 'text-slate-400'}>
                          {zone.title}
                        </span>
                      </div>
                    ))}
