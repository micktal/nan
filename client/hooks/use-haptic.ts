...
export function useHaptic() {
  ...
  const simulateVisualFeedback = useCallback(
    (element: HTMLElement, type: HapticType) => {
      if (!element) return;
  ...
  const triggerHaptic = useCallback(
    (type: HapticType, element?: HTMLElement) => {
      ...
      if (isVibrationSupported()) {
        try {
          if (Array.isArray(pattern)) {
            ...
            navigator.vibrate(
              Math.round(pattern * configRef.current.intensity),
            );
          }
        } catch (error) {
  ...
  const triggerLight = useCallback(
    (element?: HTMLElement) => triggerHaptic("light", element),
    [triggerHaptic],
  );

  const triggerMedium = useCallback(
    (element?: HTMLElement) => triggerHaptic("medium", element),
    [triggerHaptic],
  );

  const triggerHeavy = useCallback(
    (element?: HTMLElement) => triggerHaptic("heavy", element),
    [triggerHaptic],
  );

  const triggerSuccess = useCallback(
    (element?: HTMLElement) => triggerHaptic("success", element),
    [triggerHaptic],
  );

  const triggerWarning = useCallback(
    (element?: HTMLElement) => triggerHaptic("warning", element),
    [triggerHaptic],
  );

  const triggerError = useCallback(
    (element?: HTMLElement) => triggerHaptic("error", element),
    [triggerHaptic],
  );
