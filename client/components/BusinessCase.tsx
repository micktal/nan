...
export function BusinessCase() {
  const testimonials = [
    {
      ...
      quote: "Réduction de 73% des accidents depuis l'implémentation. Le ROI a été atteint en seulement 3 mois. Une solution révolutionnaire pour notre industrie.",
      metrics: {
        roi: "340%",
        accidents: "-73%", 
        compliance: "99%",
    ...
    },
    {
      ...
      quote: "L'impact sur notre culture sécurité est extraordinaire. Formation engageante, suivi précis, économies substantielles. Exactement ce dont Gerflor a besoin.",
      metrics: {
        roi: "285%",
        accidents: "-58%",
        compliance: "97%", 
  ...
  const industryBenchmarks = [
    { metric: "Réduction accidents", industry: "35%", solution: "67%", improvement: "+91%" },
    { metric: "Temps formation", industry: "4h", solution: "45min", improvement: "-81%" },
    { metric: "Coût par employé", industry: "180€", solution: "85€", improvement: "-53%" },
    { metric: "Taux completion", industry: "72%", solution: "94%", improvement: "+31%" }
  ];
      ...
      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {testimonials.map((testimonial, index) => (
            <Card key={index} className="bg-slate-800/50 border-slate-600/50 backdrop-blur-md">
              <CardHeader>
              ...
              <CardContent>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="text-center p-2 bg-emerald-500/10 rounded">
                    <div className="text-emerald-400 font-bold">{testimonial.metrics.roi}</div>
                    <div className="text-xs text-slate-400">ROI</div>
                  </div>
                  <div className="text-center p-2 bg-red-500/10 rounded">
                    <div className="text-red-400 font-bold">{testimonial.metrics.accidents}</div>
                    <div className="text-xs text-slate-400">Accidents</div>
                  </div>
                  <div className="text-center p-2 bg-blue-500/10 rounded">
                    <div className="text-blue-400 font-bold">{testimonial.metrics.compliance}</div>
                    <div className="text-xs text-slate-400">Conformité</div>
                  </div>
                  <div className="text-center p-2 bg-yellow-500/10 rounded">
                    <div className="text-yellow-400 font-bold">{testimonial.metrics.satisfaction}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {industryBenchmarks.map((benchmark, index) => (
              <div key={index} className="text-center p-4 bg-white/5 rounded-lg">
                <div className="text-sm text-slate-400 mb-2">{benchmark.metric}</div>
                <div className="space-y-1">
                  <div className="text-xs text-slate-500">Marché: {benchmark.industry}</div>
                  <div className="text-lg font-bold text-white">{benchmark.solution}</div>
                  <div className="text-sm font-bold text-emerald-400">{benchmark.improvement}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <Card className="bg-gradient-to-r from-emerald-900/30 to-blue-900/30 border-emerald-500/50 backdrop-blur-md">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h4 className="text-lg font-bold text-white mb-4">Notre Solution vs Concurrence</h4>
            ...
            <div>
              <h4 className="text-lg font-bold text-white mb-4">Bénéfices Exclusifs Gerflor</h4>
              <div className="space-y-3">
                <div className="flex items-center gap-3 p-3 bg-emerald-500/10 rounded-lg">
                  <Shield className="w-5 h-5 text-emerald-400" />
                  <span className="text-slate-300">Solution sur-mesure industrie</span>
                </div>
                <div className="flex items-center gap-3 p-3 bg-blue-500/10 rounded-lg">
                  <TrendingUp className="w-5 h-5 text-blue-400" />
                  <span className="text-slate-300">ROI calculé en temps réel</span>
                </div>
                ...
                <div className="flex items-center gap-3 p-3 bg-orange-500/10 rounded-lg">
                  ...
                  <span className="text-slate-300">Garantie résultats 90 jours</span>
                </div>
              </div>
            </div>
          </div>
